...
class Articles():
    ...
    def _parse_article(self, article: ET.Element):
        ...
        #Question 3: on HW. We fix PubDate and DateCompleted here
        ...
        for el in article.iter():
            if el.tag in tags:
                if(el.tag == 'PubDate' or el.tag == 'DateCompleted'):
                    # need to go deeper
                    year = el.findtext('Year')
                    month = el.findtext('Month')
                    day = el.findtext('Day')
                    row[el.tag] = f"{year}-{month}-{day}"
                else:
                    row[el.tag] = el.text
...
if __name__ == '__main__':
    ...
    print(articles.get_entries().columns)
    print("#---------------------#")
